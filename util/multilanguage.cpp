...
#pragma warning(pop)
    ...
    // Default data
    static const U16* defaultData = {nullptr};
        ...
        while ((sScope = fScope->NextFunction()) != nullptr)
        {
            switch (sScope->NameCrc())
            {
                case 0x971E8A6A: // "LoadLanguage"
                    LoadLanguage(StdLoad::TypeString(sScope), nullptr);
                    break;
        ...
        if (Crc::CalcStr(language) == 0x335DFA3A) // "All"
        {
            ...
            do
            {
                if (!(find.finddata.attrib & File::SUBDIR))
                {
                    ...
                    LoadLanguage(nullptr, find.finddata.name);
                }
            }

...
public:
    ...
    {
        for (typename NBinTree<KeyInfo>::Iterator i(&xTree); *i; ++i)
        {
            if ((*i)->index == index)
            {
                return (*i);
    ...
    {
        for (typename NBinTree<KeyInfo>::Iterator i(&yTree); *i; ++i)
        {
            if ((*i)->index == index)
            {
                return (*i);
    ...
    Bool GetXIndex(const char* key, U8& index)
    {
        ...
        {
            // Create the key
            info = new KeyInfo(key, static_cast<U8>(xCount));
    ...
    Bool GetYIndex(const char* key, U8& index)
    {
        ...
        if (!info && yCount < yMax)
        {
            ...
            // Create the key
            info = new KeyInfo(key, static_cast<U8>(yCount));

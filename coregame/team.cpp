...
{
    // Constructor
    TeamSubstitution() : Substitution(TEAM_VARCHAR)
    {
    }
    ...
    resources("Resources")
{
    for (U32 r = 0; r < Relation::MAX; r++)
    {
        ...
        char name[256];
        Utils::Strcpy(name, Relation(static_cast<Relation::Type>(r)).GetName());
...
void Team::Setup()
{
    ...
    // Create vars
    VarSys::CreateString("@.engine.name", GetName(), VarSys::NOEDIT, nullptr, this);
}
...
void Team::Process()
{
    ...
    if (powerLevelUnfiltered != powerLevelNewUnfiltered)
    {
        ...
        GetRadio().Trigger(Crc::CalcStr(event), nullptr);
    }
...
void Team::ReportKill(MapObj* mapObj)
{
    ...
    if (!mapObj->MapType()->HasProperty(0xBE0A35C9)) // "Team::NoStats"
    {
        if (unitObj && (team = unitObj->GetTeam()) != nullptr)
        {
...
void Team::AddResourceStore(U32 mod)
{
    ...
    NotifyPlayer(nullptr, 0xD45DC3A0); // "Team::AddResource"

...
namespace FootPrint
{
    ...
    void Placement::AdjustLocation(Matrix& location)
    {
        ...
        location.posit.x += static_cast<F32>(cellX * WC_CELLSIZE) - pos.x;
        location.posit.z += static_cast<F32>(cellZ * WC_CELLSIZE) - pos.z;
    }
        ...
        for (z = 0; z < zMax; z++)
        {
            for (x = 0; x < xMax; x++)
            {
                ...
                if (WorldCtrl::CellOnMapNoEdge(cell.map.x, cell.map.z))
                {
                    ...
                    TerrainData::Cell* thumpCell = thumped.IsValid() ? thumped.GetCell(x, z) : nullptr;
                    ...
                    else if (!TestThumpArea(cell.map, cell.map))
                    {
                        r = PR_THUMPING;
                    }
                    else
    ...
    void Placement::Thumped::UpdateCells()
    {
        ...
        for (S32 z = 0; z < localMax.z; z++)
        {
            for (S32 x = 0; x < localMax.x; x++)
            {
                ...
                c.slope = static_cast<U8>(asin(fabs(v.Dot(norm))) * RAD2DEG);
            }
        }
    }

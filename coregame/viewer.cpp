...
namespace Viewer
{
    ...
    static FScope* configScope = nullptr;
    ...
    void Init()
    {
        ...
        if (Common::InStudio())
        {
            ...
            VarSys::CreateFloat("camera.gameconfig.height", gameCfgHeight, VarSys::DEFAULT, &configHeight)->
                SetFloatRange(MIN_HEIGHT, MAX_HEIGHT);
            VarSys::CreateFloat("camera.gameconfig.pitchratio", gameCfgPitchRatio, VarSys::DEFAULT, &configPitchRatio)->
                SetFloatRange(RATIO_MIN, RATIO_MAX);
        }
    ...
    void Move(const Vector& pos, F32 yaw, F32 pitch)
    {
        ...
        // Update camera
        current->Set(pos, q, Base::SET_SMOOTH);
    }

...
namespace Conditions
{
    ...
    void Type::PostLoad()
    {
        ...
        else
        {
            ...
            if ((type = GameObjCtrl::FindType<MapObjType>(typeName.str)) == nullptr)
            {
    ...
    Bool TagCondition::TagTest(Team* team)
    {
        ...
        if (tag->tag.Alive())
        {
            ...
            for (MapObjList::Iterator o(&tag->tag->list); *o; ++o)
            {
                ...
                UnitObj* parentObj = obj->GetParent()
                                         ? Promote::Object<UnitObjType, UnitObj>(obj->GetParent())
                                         : nullptr;
    ...
    Bool InRegion::Test(Team* team)
    {
        ...
        return (RegionTest(team, nullptr));
    }
    ...
    ConstructingType::ConstructingType(FScope* fScope) :
        Condition(fScope)
    {
        if ((unitType = GameObjCtrl::FindType<UnitObjType>(StdLoad::TypeString(fScope, "Type"))) == nullptr)
        {
    ...
    private:
        ...
        {
            return (crc
                        ? team->GetEndGame().kills[Relation::ALLY]->GetAmount(crc)
        ...
        {
            return (crc
                        ? team->GetEndGame().kills[Relation::NEUTRAL]->GetAmount(crc)
        ...
        {
            return (crc
                        ? team->GetEndGame().kills[Relation::ENEMY]->GetAmount(crc)
    ...
    //
    /*template <> */
    VarConst<F32>::VarConst<F32>(FScope* fScope)
        : Condition(fScope),
    ...
    //
    /*template <> */
    VarConst<U32>::VarConst<U32>(FScope* fScope)
        : Condition(fScope),
          name(StdLoad::TypeString(fScope)),
          oper(StdLoad::TypeString(fScope)),
    ...
    //
    template <>
    Bool VarConst<U32>::Test(Team* team)
    {
        ASSERT(team);
    ...
    //
    template <>
    Bool VarConst<F32>::Test(Team* team)
    {
        ASSERT(team);
    ...
    //
    /*template <>*/
    VarVar<F32>::VarVar<F32>(FScope* fScope)
        : Condition(fScope),
          name(StdLoad::TypeString(fScope)),
          oper(StdLoad::TypeString(fScope)),
    ...
    //
    /*template <> */
    VarVar<U32>::VarVar<U32>(FScope* fScope)
        : Condition(fScope),
          name(StdLoad::TypeString(fScope)),
          oper(StdLoad::TypeString(fScope)),
          var(StdLoad::TypeString(fScope))

...
const F32 WC_CELLSIZEF32 = static_cast<F32>(WC_CELLSIZE);
...
const F32 WC_CLUSTERSIZEINCELLSF = static_cast<F32>(WC_CLUSTERSIZEINCELLS);
...
public:
    ...
    // Returns TRUE if the specified cell position is on the map
    static Bool CellOnMapX(S32 x) { return (x >= 0 && x < static_cast<S32>(cellMapX)); }
    static Bool CellOnMapZ(S32 z) { return (z >= 0 && z < static_cast<S32>(cellMapZ)); }
    ...
    // Returns TRUE if the specified cell position is on the map
    static Bool CellOnMapXNoEdge(S32 x) { return (x >= 1 && x < static_cast<S32>(cellMapX - 1)); }
    static Bool CellOnMapZNoEdge(S32 z) { return (z >= 1 && z < static_cast<S32>(cellMapZ - 1)); }
    ...
    static void ClampPlayFieldCellX(U32& val)
    {
        ...
        else if (val < playFieldCells.p0.x)
        {
    ...
    static void ClampPlayFieldCellZ(U32& val)
    {
        ...
        else if (val < playFieldCells.p0.z)
        {
    ...
    {
        ASSERT(CellOnMapX(x));
        return ((static_cast<F32>(x) + 0.5f) * WC_CELLSIZEF32);
    }
    ...
    {
        ASSERT(CellOnMapZ(z));
        return ((static_cast<F32>(z) + 0.5f) * WC_CELLSIZEF32);
    }
...
{
    return ((GetIndex() < (WorldCtrl::clusterCount - 1)) ? this + 1 : nullptr);
}

// Returns the previous X cluster
MapCluster* MapCluster::GetPreviousX()
{
    return ((xIndex > 0) ? this - 1 : nullptr);
}

// Returns the next X cluster
MapCluster* MapCluster::GetNextX()
{
    return ((xIndex < (WorldCtrl::clusterMapX - 1)) ? this + 1 : nullptr);
}

// Returns the previous Z cluster
MapCluster* MapCluster::GetPreviousZ()
{
    return ((zIndex > 0) ? this - WorldCtrl::clusterMapX : nullptr);
}

// Returns the next Z cluster
MapCluster* MapCluster::GetNextZ()
{
    return ((zIndex < (WorldCtrl::clusterMapZ - 1)) ? this + WorldCtrl::clusterMapX : nullptr);
}

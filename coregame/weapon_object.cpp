...
namespace Weapon
{
    ...
    U32 Object::FiringSolution(Vector& solution, Vector& objSolution, F32& fParam, const Target& target)
    {
        ...
        else
        {
            ...
            if ((type.flags & Flags::AimAtBox) && (target.GetType() == Target::OBJECT))
            {
                ...
                if (Test(target.GetObj(), position, targetVector, dist, Ray::BOX))
                {
    ...
    Bool Object::CalculateArc(const Vector& deltaPos, F32 speed, Vector& velocity, F32& time)
    {
        ...
        invTime = static_cast<F32>(sqrt(velocityXZ2 / positXZ2));
    ...
    void Object::LaunchProjectile(Matrix& m, F32 speed)
    {
        if (HaveAmmunition())
        {
            ...
            ProjectileObj* projectile = static_cast<ProjectileObj*>(type.projectileType->NewInstance(0));
        ...
        if (IsAllowedMovement())
        {
            ...
            if (task)
            {
                ...
                else
                {
                    ...
                    if (QueryProperty(task->GetTaskTable(), &unit, Tactical::TP_HOLDPOSITION))
                    {
                        BreakOffAttack();
                        return;
                    }
                    ...
                    if (!QueryProperty(task->GetTaskTable(), &unit, Tactical::TP_FOLLOWSUBJECT))
                    {
            ...
            {
                unit.GetDriver()->SetupDirectControl(FALSE);
                unit.Move(moveHandle, &target.GetLocation(), nullptr, NULL);
    ...
    Bool Object::FireCallBack(MapObj* mapObj, FX::CallBackData& cbd, void* context)
    {
        ...
        Object* weapon = unitObj->GetWeapon();
        ASSERT(weapon);
    ...
    void Object::Process()
    {
        ...
        switch (state)
        {
            ...
            case State::Waiting:
                ...
                if (GameTime::SimTotalTime() - lastWaitTime > MinimumWaitDelay)
                {
                    ...
                    unit.Move(moveHandle, &guardPosition, nullptr, NULL);
        ...
        if (!target.Alive() || !target.Valid())
        {
            ...
            if (
                ...
                !QueryProperty(task->GetTaskTable(), &unit, Tactical::TP_FOLLOWSUBJECT))
            {
                // Return to the guard position
        ...
        switch (type.style)
        {
            case Style::Constant:
                ...
                if (!damageApplied)
                {
                    if (type.numFirePoints)
                    {
                        ...
                        for (U32 f = 0; f < type.numFirePoints; f++)
                        {
                            ...
                            unit.StartGenericFX(0xE87BF766, FireCallBack, TRUE, nullptr, node); // "Weapon::Fire"
                        }
                    }
                    else
                    {
                ...
                if (target.GetType() == Target::OBJECT)
                {
                    ...
                    F32 damage = static_cast<F32>(GetDamage(mapObj->MapType()->GetArmourClass()));
                    F32 time = GameTime::SimTotalTime() - firingStartTime;
                    S32 total = static_cast<S32>(time * damage);
            ...
            case Style::Instant:
            {
                ...
                if (target.GetType() == Target::OBJECT)
                {
                    ...
                    mapObj->StartGenericFX(0x4A2533FF, nullptr, TRUE/*, &dir*/); // "Weapon::Hit"
                }
                else
                {
    ...
    void Object::BreakOffAttack()
    {
        ...
        (
            IsAllowedMovement() &&
            task &&
            !QueryProperty(task->GetTaskTable(), &unit, Tactical::TP_HOLDPOSITION) &&
            !QueryProperty(task->GetTaskTable(), &unit, Tactical::TP_FOLLOWSUBJECT) &&

...
namespace Radio
{
        ...
        if (config)
        {
            ...
            for (NList<Message>::Iterator m(&config->messages); *m; ++m)
            {
                ...
                if (allowed)
                {
                        ...
                        case 0x5BDE6FB6: // "ObjMessage::Neutral"
                            ...
                            if (event.GetFlags() & Event::OBJECT)
                            {
                                ...
                                (
                                    ...
                                    (event.GetFlags() & Event::LOCATION) ? &event.GetLocation() : nullptr,
                                    numParam
                                );
                            }
                            else
                    ...
                    for (p = 0; p < static_cast<S32>(numParam); p++)
                    {
                        __asm
                            {
                            pop eax
    ...
    void EventQueue::UnInstallReceiver(void* context)
    {
        ...
        while ((list = l++) != nullptr)
        {
            Receiver* receiver;
            NList<Receiver>::Iterator r(list);

            while ((receiver = r++) != nullptr)
            {
    ...
    Message::Message(FScope* fScope)
    {
        ...
        const char* param;
        while ((param = StdLoad::TypeStringD(fScope, nullptr)) != nullptr)
        {

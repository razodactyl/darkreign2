...
namespace Network
{
    ...
    namespace Ping
    {
        ...
        public:
            ...
            // Send a ping
            void Send(Handle& handle) override;
        ...
        // DLL
        HINSTANCE IPHelperAPI::dll = nullptr;

        // Get RttAndHopCount function call
        IPHelperAPI::GetRTTAndHopCountFunc IPHelperAPI::GetRTTAndHopCount = nullptr;
...
#define CHECKFUNC(x) if (!x) { LERR("Could not find '" #x "' in ws2_32.dll") };
        ...
        U32 STDCALL IPHelperAPI::ThreadProc(void* context)
        {
            ...
            events.AddEvent(iphelper->eventAdd, nullptr);
        ...
        void RawSocket::Send(Handle& handle)
        {
            ...
            header->code = 0;
            header->checksum = 0;
            header->id = static_cast<U16>(GetCurrentProcessId());
            header->seq = static_cast<U16>(seq->out);
        ...
        U32 STDCALL RawSocket::ThreadProc(void* context)
        {
            ...
            Bool quit = FALSE;
            Win32::EventIndex::List<2> events;
            events.AddEvent(pingEvent, nullptr);

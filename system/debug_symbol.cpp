...
#define SYM_PATH                 "library\\dbg\\"
        ...
        typedef Bool (STDCALL *SymEnumerateModulesFunc)
        (
            HANDLE process, SymEnumModulesCallbackFunc callback,
            void* context
        );
        typedef Bool (CALLBACK *SymEnumSymbolsCallbackFunc)(char* symbol, U32 address, U32 size, void* context);
        typedef Bool (STDCALL *SymEnumerateSymbolsFunc)
        (
            HANDLE process, U32 base, SymEnumSymbolsCallbackFunc callback,
            void* context
        );
        ...
        const char* Expand(U32 address)
        {
            ...
            Expand
            (
                address, functionName, functionDisplacement, fileName, lineNumber, lineNumberDisplacement,
                moduleName
            );
            ...
            Utils::Sprintf
            (
                buffer, 2048, "%08X %s %s [%04X] %s(%d) [%04X]",
                address,
                moduleName,
        ...
        Bool CALLBACK SymEnumModulesCallback(char* module, U32 address, void* context)
        {
            ...
            U32* total = static_cast<U32*>(context);
            U32 size = 0;

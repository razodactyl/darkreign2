...
#define T64 0xeb86d391
    ...
    static void Process(State& state, const U8* data /*[64]*/)
    {
        ...
        if (!((data - static_cast<const U8*>(nullptr)) & 3))
        {
...
#undef SET
    ...
    void Append(State& state, const U8* data, U32 size)
    {
        ...
        U32 nbits = static_cast<U32>(size << 3);
    ...
    void Finish(State& state, U8 digest[16])
    {
        ...
        for (i = 0; i < 8; ++i)
        {
            data[i] = static_cast<U8>(state.count[i >> 2] >> ((i & 3) << 3));
        }
        ...
        for (i = 0; i < 16; ++i)
        {
            digest[i] = static_cast<U8>(state.abcd[i >> 2] >> ((i & 3) << 3));
        }
    }
}

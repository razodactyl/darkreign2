...
public:
    ...
    public:
        ...
        Key(HKEY base, String name, Access access = ALL, Bool create = TRUE)
        {
            ...
            if (create)
            {
                if (RegCreateKeyEx(base, name, NULL, "", REG_OPTION_NON_VOLATILE, access, nullptr, &key, &disp) != ERROR_SUCCESS)
                {
                    MessageBox(nullptr, Debug::LastError(), "RegCreateKeyEx error", MB_OK | MB_ICONSTOP);
                }
            }
            else
            {
        ...
        {
            if (RegCreateKeyEx(base.key, name, NULL, "", REG_OPTION_NON_VOLATILE, access, nullptr, &key, &disp) != ERROR_SUCCESS)
            {
                MessageBox(nullptr, Debug::LastError(), "RegOpenKeyEx error", MB_OK | MB_ICONSTOP);
            }
        }
        ...
        {
            if (RegQueryValueEx(key, name, nullptr, (U32*)type, data, length) != ERROR_SUCCESS)
            {
        ...
        Bool Enum(U32 index, String& s)
        {
            ...
            switch (RegEnumKeyEx(key, index, buff, &length, nullptr, nullptr, nullptr, nullptr))
            {
                case ERROR_SUCCESS:
                    s.Dup(buff);
                    return (TRUE);
                ...
                default:
                    MessageBox(nullptr, Debug::LastError(), "RegEnumKeyEx error", MB_OK | MB_ICONSTOP);
                    return (FALSE);
                    break;
            }
        }
        ...
        Bool EnumValue(U32 index, String& s)
        {
            ...
            switch (RegEnumValue(key, index, buff, &length, nullptr, nullptr, nullptr, nullptr))
            {
                case ERROR_SUCCESS:
                    s.Dup(buff);
                    return (TRUE);

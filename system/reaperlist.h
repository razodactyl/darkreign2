...
class ReaperList : public NList<NODE>
{
public:
    ...
    NODE* operator[](int number) const
    {
        if (GetHeadNode())
        {
            typename NList<NODE>::Node* node = GetHeadNode();
    ...
    DATA* RotateBackward()
    {
        typename NList<NODE>::Node* node = GetTailNode();
    ...
    NODE* Prepend(DATA* data = nullptr)
    {

...
namespace Mono
{
    ...
    static HANDLE screenHandle = nullptr;
...
#define IOCTL_MAPMEM_UNMAP_USER_PHYSICAL_MEMORY CTL_CODE(FILE_DEVICE_MAPMEM, MAPMEM_IOCTL_INDEX + 1, METHOD_BUFFERED, FILE_ANY_ACCESS)
    ...
    static Bool MapMemNT()
    {
        ...
        (
            ...
            (
                screenHandle,
                static_cast<U32>(IOCTL_MAPMEM_MAP_USER_PHYSICAL_MEMORY),
                &pmi,
                sizeof(pmi),
                &scrnPtr,
                sizeof(PVOID),
    ...
    void UnmapMemNT()
    {
        if (screenHandle)
        {
            ...
            (
                screenHandle,
                static_cast<U32>(IOCTL_MAPMEM_UNMAP_USER_PHYSICAL_MEMORY),
                &scrnPtr,
                sizeof(PVOID),
                nullptr,
                0,
    ...
    void Init()
    {
        ...
        lock = CreateEvent(nullptr, FALSE, FALSE, "DarkReign2::Mono::Lock");
    ...
    void Update(S32 fps, F32 elapFrame, S32 mousex, S32 mousey, U32 triCount)
    {
        ...
        if (triCount)
        {
            ...
            U32 trisPerSec = elapFrame > 1e-4 ? static_cast<U32>(static_cast<F32>(triCount) * 1000.0f / elapFrame) : 0;
            ...
            case WM_KEYDOWN:
            {
                ...
                if (ctrlDown && winDown)
                {
                    ...
                    if (vk == VK_HOME)
                    {
                        ...
                        while (*scratchItr)
                        {
                            Scratch::Panel* p = static_cast<Scratch::Panel*>(*scratchItr);
    ...
    void Console::Write(const char* str, EAttrib attrib)
    {
        ...
        while (*str)
        {
            ...
            while (i++ < length && *str)
            {
                ...
                buffer[offset++] = *str++;
                buffer[offset++] = static_cast<U8>(attrib);
            }
    ...
    void Buffer::Write(S32 row, S32 col, const char* str, EAttrib attrib)
    {
        ...
        {
            buffer[i++] = *str++;
            buffer[i++] = static_cast<U8>(attrib);
        }
    ...
    void Buffer::FillRow(S32 row, S32 ch, EAttrib attrib)
    {
        ...
        do
        {
            buffer[offset + i++] = static_cast<U8>(ch);
            buffer[offset + i++] = static_cast<U8>(attrib);
        }
        while (i < 160);
    ...
    void Buffer::PutChar(S32 row, S32 col, S32 ch)
    {
        ...
        buffer[offset] = static_cast<U8>(ch);
    ...
    void Buffer::PutChar(S32 row, S32 col, EAttrib attrib, S32 ch)
    {
        ...
        buffer[offset] = static_cast<U8>(ch);
        buffer[offset + 1] = static_cast<U8>(attrib);
    ...
    void Scratch::Create(U32 id, const char* owner, const char* name)
    {
        ...
        Panel* panel = new Panel(viewPort.right - viewPort.left + 1, viewPort.bottom - viewPort.top + 1, owner);
    ...
    {
        Buffer* p = static_cast<Buffer*>(scratchPanels.Find(id));

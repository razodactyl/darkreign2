...
namespace FileSys
{
    ...
    {
        // Map portion of file
        memoryPtr = static_cast<U8*>(source->packFile.MapMemory
            (
                name.str, File::MAP_READ, info->offset, info->size,
                &alignedPtr
            ));
    }
    ...
    FastFind* FileSrcPack::GetFastFind(const char* name, ResourceStream* stream)
    {
        ...
        // Setup fast find
        return (info ? new FastFindPack(name, info->size, stream, this, info) : nullptr);
    }
    ...
    DataFile* FileSrcPack::Open(const char* name)
    {
        ...
        // Open the file
        return (info ? new DataFilePack(name, this, info) : nullptr);
    };

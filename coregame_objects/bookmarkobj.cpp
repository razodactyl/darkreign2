...
//
BookmarkObj* BookmarkObj::CreateBookmark(const char* bookMarkName, Type subTypeIn)
{
    ...
    if (!FindBookmark(bookMarkName))
    {
        ...
        if (BookmarkObjType* type = GameObjCtrl::FindType<BookmarkObjType>("BookMark"))
        {
            ...
            BookmarkObj* obj = static_cast<BookmarkObj*>(type->NewInstance(0));
...
void BookmarkObj::LoadState(FScope* fScope)
{
    ...
    if ((sScope = fScope->GetFunction("Camera", FALSE)) != nullptr)
    {
        subType = POSITION;
    ...
    else if ((sScope = fScope->GetFunction("Curve", FALSE)) != nullptr)
    {
        ASSERT(!curveSrc);
        ASSERT(!curve);
        ...
        if ((ssScope = sScope->GetFunction("Focus", FALSE)) != nullptr)
        {
...
//
void BookmarkObj::Curve::Build(CurveSrc& list, Bool loopIn)
{
    // Save flags
    loop = loopIn;
    totalTime = 0.0F;

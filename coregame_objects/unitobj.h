...
public:
    ...
    Bool DefaultPrimitiveProcessing() override
    {
        return (TRUE);
    }
    ...
    UnitObj* SpawnClosest(const Vector& pos, Team* team = nullptr, Bool construct = FALSE, F32 orientation = 0.0F);

    // Create a unit at the closest available linked location, or return NULL
    UnitObj* SpawnLinked(const Vector& pos, Team* team = nullptr, Bool construct = FALSE, F32 orientation = 0.0F);

    // Create a unit at the given location
    UnitObj& Spawn(const Vector& pos, Team* team = nullptr, Bool construct = FALSE, F32 orientation = 0.0F);
    UnitObj& Spawn(const Matrix& mat, Team* team = nullptr, Bool construct = FALSE);
...
public:
    ...
    {
        return (upgradeFor.Alive() ? upgradeFor.GetData() : nullptr);
    }
...
public:
    ...
    void MoveOnto(Movement::Handle& handle, UnitObj* obj, Bool passUnit, Movement::RequestData* req = nullptr);
    ...
    // AddToMapHook
    void AddToMapHook() override;

    // RemoveFromMapHook
    void RemoveFromMapHook() override;

    // Capture/Release cell hooks when the object's centre point moves cells
    void CaptureCellHooks(Bool capture) override;

    // Per-cycle processing
    void ProcessCycle() override;

    // Capture and release map hooks
    void CaptureMapHooks(Bool capture) override;
    ...
    Bool FindCover(Bool search, UnitObj* avoid = nullptr);

    // Find the given source of danger
    Bool FindDanger(UnitObj* sourceUnit = nullptr);

    // Is it time for this unit to respond to danger
    Bool ReadyToRespondToDanger();

    // Give this unit the chance to respond to danger
    Bool DangerResponse(UnitObj* sourceUnit = nullptr, UnitObj* buddy = nullptr);
...
public:
    ...
    Team* GetActiveTeam()
    {
        return (IsActivelyOnTeam() ? GetTeam() : nullptr);
    }
    ...
    {
        // This is a safe cast
        return static_cast<UnitObjType*>(type);
    };

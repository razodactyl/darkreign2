...
namespace Studio
{
    ...
    namespace Brush
    {
                               ...
                               Bool align)
        {
            ...
            SegmentListNode* insertPoint = nullptr;
        ...
        void Curve::NotifyVar(IFaceVar* var)
        {
            ...
            else if (var == varStrength || var == varTime)
            {
                for (U32 count = 0; count < 2; count++)
                {
                    SegmentList* list = count ? &focusList : &curveList;
            ...
            switch (crc)
            {
                ...
                case 0x9031A85E: // "Brush::Objects::ObjectCreated"
                {
                    if (lastCreatedObj.Alive())
                    {
                        ...
                        if ((marker = Promote::Object<MarkerObjType, MarkerObj>(lastCreatedObj)) != nullptr)
                        {
                            SegmentList* list;
                ...
                case 0xBFF8BC7F: // "Brush::Curve::Message::Info"
                {
                    ...
                    for (MapObjList::Iterator i(&data.sList); *i; ++i)
                    {
                        ...
                        if (obj && ((node = FindNode(obj)) != nullptr))
                        {
        ...
        Bool Curve::CameraSimulate(FamilyNode* node, F32 time, void* context)
        {
            ...
            Curve* brush = static_cast<Curve*>(context);
            Vector v, t;
            ...
            else if (brush->varPlayMode->GetIntegerValue())
            {
                currTime += time;
            }

...
namespace Client
{
    ...
    Color ClusterMap::CallbackResource(void*, U32 x, U32 y)
    {
        ...
        if (value)
        {
            F64 fvalue = 11.090354889 * log(static_cast<F64>(value));

            if (fvalue < 256)
            {
                return (Color(0, static_cast<U32>(fvalue), 0));
            }
            fvalue -= 256;
            return (Color(static_cast<U32>(fvalue), 255, static_cast<U32>(fvalue)));
        }
    ...
    {
        Occupation* occupation = static_cast<Occupation*>(context);
        MapCluster* cluster = WorldCtrl::GetCluster(x, y);
        U32 value;
    ...
    {
        Threat* threat = static_cast<Threat*>(context);
        MapCluster* cluster = WorldCtrl::GetCluster(x, y);
        U32 value;
        ...
        {
            value = AI::Map::GetThreat(threat->team, threat->armourClass)
                        ? cluster->ai.GetThreat(threat->team, threat->armourClass)
                        : 0;
        }

        if (value)
        {
            F64 fvalue = 16.6355323334 * log(static_cast<F64>(value));

            if (fvalue < 256)
            {
                return (Color(static_cast<U32>(fvalue), 0, 0));
            }
            if (fvalue < 512)
            {
                return (Color(255, static_cast<U32>(fvalue - 256), 0));
            }
            return (Color(255, 255, static_cast<U32>(fvalue - 512)));
        }
        return (Color(0.0f, 0.0f, 0.0f));
    }
    ...
    {
        Defense* defense = static_cast<Defense*>(context);
        MapCluster* cluster = WorldCtrl::GetCluster(x, y);
        U32 value;
        ...
        if (defense->damageId == CLUSTERMAP_DEFENSE_ALL)
        {
            value = AI::Map::GetTotalDefense(defense->team)
                        ? cluster->ai.GetTotalDefense(defense->team) * 768 / AI::Map::GetTotalDefense(defense->team)
                        : 0;
        }
        else
        {
    ...
    {
        Pain* pain = static_cast<Pain*>(context);
        MapCluster* cluster = WorldCtrl::GetCluster(x, y);
        U32 value;
        ...
        {
            value = AI::Map::GetPain(pain->team, pain->armourClass)
                        ? cluster->ai.GetPain(pain->team, pain->armourClass)
                        : 0;
        }
        ...
        if (value)
        {
            ...
            if (fvalue < 512)
            {
                return (Color(0, 255, static_cast<U32>(fvalue - 256)));
            }
            return (Color(static_cast<U32>(fvalue - 512), 255, 255));
        }
        return (Color(0.0f, 0.0f, 0.0f));
    }
    ...
    {
        Region* region = static_cast<Region*>(context);
        MapCluster* cluster = WorldCtrl::GetCluster(x, y);
                ...
                else
                {
                    switch (Crc::CalcStr(type))
                    {
                        case 0x4CD1BE27: // "resource"
                        {
                            ...
                            maps.Add(Crc::CalcStr("Resources"), new Map(grid, nullptr));
                            break;
                        }
                        ...
                        case 0x6FDCD1BD: // "disruption"
                        {
                            ...
                            IFace::ToggleActive(grid->Name());
                            maps.Add(Crc::CalcStr(name), new Map(grid, nullptr));
                            break;
                        }

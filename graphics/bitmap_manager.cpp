...
) //  = 0, = bitmapTEXTURE, = 0, = TRUE
{
    ...
    if (!bmp)
    {
        ...
        *dot = '.';
        ...
        //
        U32 count = 1;
        Bitmap *lbmp = nullptr, *fbmp = nullptr;
        do
        {
...
void Bitmap::Manager::MovieNextFrame()
{
    ...
    if (tail && tail->status.binkExclusive)
    {
        ...
        dxError = Vid::backBmp.GetSurface()->Blt(&backrect, tail->GetSurface(), &binkrect, DDBLT_WAIT, nullptr);
...
Bitmap* Bitmap::Manager::Load(U32 reduce, GodFile* god)
{
    ...
    {
        bitmap = FindCreate(reduce, name.str, mipMapCount, static_cast<U32>(type));
    }
    return bitmap;
}

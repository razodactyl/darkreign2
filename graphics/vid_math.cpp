...
namespace Vid
{
    ...
    void ScreenToCamera(Vector& dst, S32 sx, S32 sy)
    {
        //  dst.z = NearPlane();
        dst.z = 1.0f;
        dst.x = static_cast<F32>(sx - Math::origin.x) * dst.z * Math::projInv.x;
        dst.y = static_cast<F32>(sy - Math::origin.y) * dst.z * Math::projInv.y;
    }
    ...
    {
        return Plane(v0, v1, v2).Evalue(Math::modelViewVector) <= 0.0f;
    }
    ...
    {
        return Plane(v0, v1, v2).Evalue(Math::modelViewVector) <= 0.0f
            && Plane(v0, v2, v3).Evalue(Math::modelViewVector) <= 0.0f;
    }
    ...
    void LightTransformFromModel(VertexTL* dstV, const Vector* srcN, const Color* srcC, U32 countV)
    {
        ...
        for (VertexTL* ev = dstV + countV; dstV < ev; dstV++, srcN++, srcC++)
        {
            ...
            Light::LightModel(dstV->vv, *srcN, diffuse, *BucketMan::GetMaterial(), diff, spec);

            // set the colors
            dstV->diffuse.ModulateInline(*srcC, diff.r, diff.g, diff.b, diffuse.a);
            dstV->specular.SetInline(spec.r, spec.g, spec.b, static_cast<U32>(255));
    ...
    void LightTransformFromModel(VertexTL* dstV, const Vector* srcV, const Vector* srcN, const Color* srcC, U32 countV)
    {
        ...
        for (VertexTL* ev = dstV + countV; dstV < ev; dstV++, srcV++, srcN++, srcC++)
        {
            ...
            Light::LightModel(*srcV, *srcN, diffuse, *BucketMan::GetMaterial(), diff, spec);

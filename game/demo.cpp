...
namespace Demo
{
    ...
    static NotifyProc* notifyProc = nullptr;
    ...
    void Process()
    {
        ...
        switch (state)
        {
            case PLAYING:
            {
                ...
                else if (GameTime::GameCycle() == demoCycles)
                {
    ...
    static void NotifyBenchmark(U32 id)
    {
        switch (id)
        {
            case 0x3B8E7EE1: // "Start"
            {
                ...
                SetFlag(IFace::DISABLE_DRAW, TRUE);
                SetFlag(IFace::DISABLE_ACTIVATE, TRUE);
            ...
            case 0xB2C53B91: // "End"
            {
                ...
                // Enable interface
                SetFlag(IFace::DISABLE_DRAW, FALSE);
                SetFlag(IFace::DISABLE_ACTIVATE, FALSE);

                // Reset game time
                GameTime::SetFastMode(FALSE);

                U32 dtime = endTime - startTime;
                F32 invdtime = 1000.0f / static_cast<F32>(dtime);
                ...
                (
                    ...
                    "%d ms\\n%.1f fps\\n%.0f tps",
                    dtime,
                    static_cast<F32>(endFrame - startFrame) * invdtime,
                    static_cast<F32>(Game::RC::TriCount()) * invdtime
                );
                Vid::LogPerf
                (
                        ...
                        if (Dir::FindFirst(find, "demos", "*.demo"))
                        {
                            do
                            {
                                if (!(find.finddata.attrib & File::SUBDIR))
                                {
                                    ...
                                    // Add to the list
                                    listBox->AddTextItem(find.finddata.name, nullptr);
                                }
                            }

...
namespace Game
{
            ...
            if (const Missions::Group* g = Missions::FindGroup(group->GetStringValue()))
            {
                ...
                if (campaign && (missionSelectionStyle & STYLE_CAMPAIGN))
                {
                    ...
                    for (NList<Campaigns::Campaign::Mission>::Iterator i(&campaign->GetMissions()); *i; ++i)
                    {
                        ...
                        // Add to the list
                        missionList->AddTextItem(record.GetMission().str, nullptr);
                ...
                else
                {
                    ...
                    {
                        missionList->AddTextItem((*i)->GetName().str, nullptr);
                    }
    ...
    void MissionSelection::InvalidatePreview()
    {
        ...
        if (preview)
        {
            ...
            {
                previewDisplay->SetImage(nullptr);
            }
                ...
                case IFace::NOTIFY:
                {
                    ...
                    switch (e.iface.p1)
                    {
                        ...
                        case MissionSelectionMsg::Save:
                        {
                            ...
                            // Save the mission
                            if (SaveMission(group->GetStringValue(), missionSave->GetStringValue()))
                            {
                                // Save the minimap

...
#define UPDATE_TEX(tex, rect)     \
    ...
    static VertexTL* GetVertHelper(U32 vertCount, U32 indexCount, const TextureInfo* tex)
    {
        ...
        // Lock vertices
        U16 vertOffset;
        VertexTL* verts = GetVerts(vertCount, tex ? tex->texture : nullptr, filter, texMode, vertOffset);

        // Automatically setup indices
        ASSERT(indexCount == 3 || indexCount == 6);

        // Setup indices
        SetIndex(Vid::rectIndices, indexCount, vertOffset);
                         ...
                         VertexTL* clip) // = NULL)
    {
      ...
      if (tex)
      {
        ...
        // top left corner
        point[0].vv.x = static_cast<F32>(localTex.texRect.p0.x);
        point[0].vv.y = static_cast<F32>(localTex.texRect.p0.y);
        point[0].vv.z = 0.0F;
        point[0].rhw = 1.0F;
        ...
        // top right corner
        point[1].vv.x = static_cast<F32>(localTex.texRect.p1.x);
        point[1].vv.y = static_cast<F32>(localTex.texRect.p0.y);
        point[1].vv.z = 0.0F;
        point[1].rhw = 1.0F;
        ...
        // bottom right corner
        point[2].vv.x = static_cast<F32>(localTex.texRect.p1.x);
        point[2].vv.y = static_cast<F32>(localTex.texRect.p1.y);
        point[2].vv.z = 0.0F;
        point[2].rhw = 1.0F;
        ...
        // bottom left corner
        point[3].vv.x = static_cast<F32>(localTex.texRect.p0.x);
        point[3].vv.y = static_cast<F32>(localTex.texRect.p1.y);
        point[3].vv.z = 0.0F;
        point[3].rhw = 1.0F;
        ...
        if (clip)
        {
            ...
            if (vCount)
            {
                ...
                SetIndex(tempI, iCount, vertOffset);
            }
        }
    }
    ...
    void RenderLine(const Point<S32>& vertex1, const Point<S32>& vertex2, Color color, F32 alphaScale)
    {
        ...
        // vertices of the rectangle
        VertexTL* point = GetVertHelper(4, 6, nullptr);
    ...
    void RenderTriangle(const Point<S32>* vertex, Color color, F32 alphaScale, VertexTL*) // = NULL)
    {
        ...
        // vertices of the rectangle
        VertexTL* point = GetVertHelper(3, 3, nullptr);

        // first left corner
        point[0].vv.x = static_cast<F32>(vertex[0].x);
        point[0].vv.y = static_cast<F32>(vertex[0].y);
        point[0].vv.z = 0.0F;
        point[0].rhw = 1.0F;
        point[0].diffuse = color;
        point[0].specular = 0xFF000000;
        ...
        // second right corner
        point[1].vv.x = static_cast<F32>(vertex[1].x);
        point[1].vv.y = static_cast<F32>(vertex[1].y);
        point[1].vv.z = 0.0F;
        point[1].rhw = 1.0F;
        point[1].diffuse = color;
        point[1].specular = 0xFF000000;
        ...
        // bottom right corner
        point[2].vv.x = static_cast<F32>(vertex[2].x);
        point[2].vv.y = static_cast<F32>(vertex[2].y);
        point[2].vv.z = 0.0F;
        point[2].rhw = 1.0F;
        point[2].diffuse = color;
        point[2].specular = 0xFF000000;
    ...
    void RenderShadow(const ClipRect& r1, const ClipRect& r2, Color color, S32 indent, F32 alphaScale)
    {
        ...
        RenderRectangle(rect, color, nullptr, alphaScale);
    ...
    void FScopeToColor(FScope* fScope, Color& c)
    {
        ...
        if (vNode)
        {
            if (vNode->aType == VNode::AT_STRING)
            {
                ...
                switch (key)
                {
                    case 0x49B99ECB: // "Black"         
                        c = Color::Std[Color::BLACK];
                        return;
                    case 0xA66B370D: // "Blue"
                        c = Color::Std[Color::BLUE];
                        return;
                    case 0x1B20CD61: // "Green"
                        c = Color::Std[Color::GREEN];
                        return;
                    case 0x1F080A8A: // "Cyan"
                        c = Color::Std[Color::CYAN];
                        return;
                    case 0x52E7A8F5: // "Red"
                        c = Color::Std[Color::RED];
                        return;
                    case 0x3AB82861: // "Magenta"
                        c = Color::Std[Color::MAGENTA];
                        return;
                    case 0xCF0E1145: // "Brown"
                        c = Color::Std[Color::BROWN];
                        return;
                    case 0x2EF357DE: // "LtGrey"
                        c = Color::Std[Color::LTGREY];
                        return;
                    case 0xCE6B1E00: // "DkGrey"
                        c = Color::Std[Color::DKGREY];
                        return;
                    case 0xF0BB1D31: // "LtBlue"
                        c = Color::Std[Color::LTBLUE];
                        return;
                    case 0xA91B17D4: // "LtGreen"
                        c = Color::Std[Color::LTGREEN];
                        return;
                    case 0x49D820B6: // "LtCyan"
                        c = Color::Std[Color::LTCYAN];
                        return;
                    case 0x763A5F1F: // "LtRed"
                        c = Color::Std[Color::LTRED];
                        return;
                    case 0x0DF86532: // "LtMagenta"
                        c = Color::Std[Color::LTMAGENTA];
                        return;
                    case 0xBDD0DC99: // "Yellow"
                        c = Color::Std[Color::YELLOW];
                        return;
                    case 0xCAB21997: // "White"
                        c = Color::Std[Color::WHITE];
                        return;
            ...
            else if (vNode && vNode->aType == VNode::AT_INTEGER)
            {
    ...
    void FScopeToTextureInfo(FScope* fScope, TextureInfo& info)
    {
        ...
        if ((info.texture = Bitmap::Manager::FindCreate(Bitmap::reduceNONE, image)) == nullptr)
        {
    ...
    void FScopeToSheetInfo(FScope* fScope, BinTree<TextureInfo>& images, Point<U8>& sheets)
    {
        ...
        for (U32 i = 0; i < U32(sheets.x * sheets.y);)
        {
            ...
            if ((sScope = fScope->NextFunction()) == nullptr)
            {
        ...
        while ((sScope = fScope->NextFunction()) != nullptr)
        {
            switch (sScope->NameCrc())
            {
                case 0x6F3CCD1D: // "ColorGroup"
                {
                    ...
                    if ((state.colors = FindColorGroup(Crc::CalcStr(name))) == nullptr)
                    {

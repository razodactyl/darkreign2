...
namespace KeyBind
{
    ...
    static Bool FindKeyByName(U32 nameCrc, U32& scanCode)
    {
        ...
        if ((keyItem = keysByNameCrc.Find(nameCrc)) != nullptr)
        {
    ...
    static Binding* CreateHold(U32 scanCode, const char* command, U32 flags)
    {
        ...
        if ((varItem = VarSys::FindVarItem(command)) != nullptr)
        {
    ...
    {
        if (pressBindings[scanCode] == nullptr)
        {
    ...
    void Init()
    {
        for (int i = 0; i < 256; i++)
        {
            pressBindings[i] = nullptr;
            keysByScan[i] = nullptr;
        }
        lastPoll = 0;

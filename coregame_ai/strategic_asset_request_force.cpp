...
namespace Strategic
{
    ...
    // Constructor
    //
    Asset::Request::Force::Force(Script* script, U32 handle, const Config::RecruitForce* config,
                                 const Point<F32>& location, F32 range, Bool acceptInsufficient, Object* object)
                    ...
                    range2)
                {
                    ...
                    for (U32 ac = 0; ac < ArmourClass::NumClasses(); ac++)
                    {
                        ...
                        for (List<Team>::Iterator t(&team->RelatedTeams(Relation::ENEMY)); *t; ++t)
                        {
                            ...
                            threat[ac] += static_cast<F32>(ai->GetThreat((*t)->GetId(), ac));
                            threatTotal += static_cast<F32>(ai->GetThreat((*t)->GetId(), ac));
                            defense[ac] += static_cast<F32>(ai->GetDefense((*t)->GetId(), ac));
                            defenseTotal += static_cast<F32>(ai->GetDefense((*t)->GetId(), ac));
                        }
                    }
                }
            }
        ...
        while ((sScope = scope->NextFunction()) != nullptr)
        {
            switch (sScope->NameCrc())
            {
                case 0x2FDBDEBB: // "Config"
                {
                    ...
                    // Find the config
                    if ((config = Config::FindRecruitForce(name.crc)) == nullptr)
                    {

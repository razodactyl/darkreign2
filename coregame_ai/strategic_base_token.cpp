        ...
        while ((sScope = scope->NextFunction()) != nullptr)
        {
            switch (sScope->NameCrc())
            {
                ...
                case 0x4CD1BE27: // "Resource"
                    if ((resource = manager.GetObject().GetResourceManager().FindResource(StdLoad::TypeU32(sScope))) ==
                        nullptr)
                    {
                ...
                case 0x6ADE7843: // "Water"
                    if ((water = manager.GetObject().GetWaterManager().FindWater(StdLoad::TypeU32(sScope))) == nullptr)
                    {
                ...
                case 0x8810AE3C: // "Script"
                    if ((script = manager.GetObject().GetScriptManager().FindScript(StdLoad::TypeU32(sScope))) ==
                        nullptr)
                    {
    ...
    void Base::Token::CheckConstructor(const UnitObjList& buildings, const UnitObjList& constructors)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                UnitObjListNode* winner = nullptr;

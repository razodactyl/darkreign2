          ...
          acceptInsufficient(acceptInsufficient)
    {
        ...
        for (; ac < ArmourClass::NumClasses(); ac++)
        {
            ...
            for (List<Team>::Iterator t(&team->RelatedTeams(Relation::ENEMY)); *t; ++t)
            {
                ...
                threat[ac] += static_cast<F32>(AI::Map::GetThreat((*t)->GetId(), ac));
                threatTotal += static_cast<F32>(AI::Map::GetThreat((*t)->GetId(), ac));
                defense[ac] += static_cast<F32>(AI::Map::GetDefense((*t)->GetId(), ac));
                defenseTotal += static_cast<F32>(AI::Map::GetDefense((*t)->GetId(), ac));
            }
        }

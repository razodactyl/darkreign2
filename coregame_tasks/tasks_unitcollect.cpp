...
#define BETTER_DISTANCE 1600.0f
        ...
        switch (notify)
        {
            case SMN_ENTRY:
                ...
                if (resourceObj.Alive())
                {
                    ...
                    subject->Move(moveHandle, &dst, nullptr, GetFlags() & TF_AI ? TRUE : FALSE, &rd);
                }
                else
                {
        ...
        switch (notify)
        {
            case SMN_ENTRY:
                ...
                if (resourceObj.Alive())
                {
                    ...
                    if (
                        (abs(static_cast<long>(subject->GetCellX()) - static_cast<long>(resourceObj->GetCellX())) <= 1)
                        &&
                        (abs(static_cast<long>(subject->GetCellZ()) - static_cast<long>(resourceObj->GetCellZ())) <= 1))
                    {
                        ...
                        subject->Move(moveHandle, nullptr, &dir, GetFlags() & TF_AI ? TRUE : FALSE);
                    }
                    else
                    {
                        LOG_COLLECT(("Adding [%d] to reject list", resourceObj->Id()))
    ...
    {
        UnitCollect* task = TaskCtrl::Promote<UnitCollect>(mapObj);
    ...
    void UnitCollect::StateGoToResourceArea(StateMachineNotify notify)
    {
        ...
        switch (notify)
        {
            case SMN_ENTRY:
                ...
                {
                    // Move to it
                    subject->Move(moveHandle, &lastResourceLocation, nullptr, GetFlags() & TF_AI ? TRUE : FALSE);
                }
                else
                {

...
#define LOG_LOCAL(x)
    ...
    {
        return (!mapObj->SendEvent(Event(0x2013F610))); // "UnitBuild::EffectProcess"
    }
    ...
    {
        // Get footprint type
        if ((foot = build->GetFootPrintType()) == nullptr)
        {
        ...
        switch (notify)
        {
            case SMN_ENTRY:
            {
                ...
                if (CheckConstruction(FALSE, placeResult))
                {
                    ...
                    target->PostEvent(Event(0x68604C50, subject)); // "UnitBuild::SetConstructor"
